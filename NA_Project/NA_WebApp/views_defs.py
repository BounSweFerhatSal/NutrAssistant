...
from .models import Diseases, Allergies, Labels
...
def diseaseSearch(request):
    ...
    query_results = Diseases.objects.filter(diseaseName__startswith=query_prm)
...
def search_allergies(request):
    ...
    # prm = request.POST.get('term', '')
    query_prm = request.POST.get('term', '')
    query_results = Allergies.objects.filter(name__startswith=query_prm)
...
def search_labels(request):
    ...
    # prm = request.POST.get('term', '')
    query_prm = request.POST.get('term', '')
    query_results = Labels.objects.filter(name__startswith=query_prm)
